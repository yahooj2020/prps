...
if __name__ == "__main__":
    f_list = []
    for item in range(1,905):

        url = 'https://pornopics.co/search/asian/{0}'.format(item)
    ...
    for single_pic in f_list:
        ...
        with open('t1.txt','a') as file_handle:   # .txt可以不自己新建,代码会自动新建
            ...
            file_handle.write('\n')         # 有时放在循环里面需要自动转行，不然会覆盖上一条数据
                ...
                # os.popen("./test.sh")
    #         rs = requests.get(f_url_p)
    #         lpath = os.getcwd()
    #         f_info = lpath+f_url_pb
            
    #         with open("{}".format(f_info),"wb") as f:
    #             f.writer(rs.content)
                
            
    # ---下载url---txt文件---shell调用txt文件下载
